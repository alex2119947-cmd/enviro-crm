...
def create_project(data):
    ...
    new_project = {
        "id": len(st.session_state.projects) + 1, "submission_date": datetime.now().strftime("%Y-%m-%d %H:%M"), "status": "На рассмотрении у инженера", "status_desc": "Ожидайте ответа...",
...
if 'current_role' not in st.session_state: st.session_state.current_role = "Новый клиент"

# --- ИСПРАВЛЕНИЕ: ЛОГИКА НАВИГАЦИИ С on_change ---
def handle_role_change():
    """Эта функция вызывается ТОЛЬКО при смене радио-кнопки."""
    st.session_state.current_project_id = None # Сбрасываем проект при смене роли
...
st.sidebar.radio(
    ...
    key="role_selector",
    on_change=handle_role_change # Вот она, "умная" навигация
)

st.sidebar.info("Версия прототипа: 3.5")
...
if st.session_state.page == "login":
    ...
    if st.button("Войти", key="login_button"):
        if password == CORRECT_PASSWORD:
            ...
            st.session_state.page = "employee_dashboard" # После входа отправляем на панель
            st.rerun()
...
# ==============================================================================
    ...
    # ... (Весь код анкет остается без изменений) ...
        ...
        for project in reversed(st.session_state.projects):
            ...
            with st.expander(f"Заявка №{project['id']} от {project['submission_date']} - {client_identifier} ({project.get('address', 'Адрес не указан')})"):
                ...
                if st.button("Просмотреть детали", key=f"details_btn_{project['id']}"):
                    ...
                    st.session_state.page = "project_page"
                    st.rerun() # Эта команда теперь будет работать правильно

# ==============================================================================
#                СТРАНИЦА ПРОЕКТА
# ==============================================================================
elif st.session_state.page == "project_page":
    # ... (Этот код без изменений, он уже работает правильно)
    ...
    if current_project is None:
        ...
        client_identifier = current_project.get('client_name') or current_project.get('company_name', 'N/A'); st.title(f"Страница проекта: {client_identifier}"); st.markdown(f"Заявка №{current_project['id']} от {current_project['submission_date']}"); st.markdown("---")
        st.subheader("1. Статус заявки"); st.success(current_project['status']); st.info(current_project['status_desc']); st.markdown("---")
        with st.expander("Показать/скрыть полные детали заявки"): display_data = current_project.copy(); display_data.pop('chat_history', None); st.json(display_data)
        st.subheader("3. Загруженные файлы")
        if "uploaded_files_info" in current_project and current_project["uploaded_files_info"]:
            for file_info in current_project["uploaded_files_info"]: size_mb = file_info['size'] / (1024 * 1024); st.info(f"📄 {file_info['name']} ({size_mb:.2f} MB)")
            st.warning("Примечание: в режиме прототипа отображается только информация о файлах, но не сами файлы.")
        else: st.write("Клиент не прикрепил файлы к этой заявке.")
        st.subheader("4. Чат по проекту")
        for message in current_project["chat_history"]:
            with st.chat_message(message["role"]): st.markdown(message["content"])
        if prompt := st.chat_input("Напишите ваш вопрос..."): current_project["chat_history"].append({"role": "user", "content": prompt}); st.rerun()
