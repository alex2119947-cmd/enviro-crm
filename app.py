...
# --- СТРАНИЦА КЛИЕНТА (ФОРМЫ) ---
    ...
    if object_type == "Частный дом":
        with st.form("private_house_form", clear_on_submit=True):
            ...
            with col4:
                coal_usage = st.number_input("Кол-во сжигаемого угля в мес. (тонн)", value=0.0, step=0.1)
                energy_usage_kwh = st.number_input("Расход кВт\*ч в мес.", value=0.0, step=10.0)
                energy_usage_som = st.number_input("Расход на энергию/отопление в мес. (сом)", value=0.0, step=100.0)
            ...
            if st.form_submit_button("Отправить заявку"):
                if not name or not phone or not address: 
                    ...
                    # <<< ИСПРАВЛЕНИЕ ЗДЕСЬ: Создаем словарь отдельно >>>
                    project_data = {
    ...
    # st.video("enviro1.mp4") # Локальное видео не будет работать на Streamlit Cloud
...
# --- СТРАНИЦА ДЕТАЛЕЙ ПРОЕКТА ---
    ...
    current_project = next(
        (p for p in st.session_state.projects if p["id"] == project_id), None
    )
    ...
    if current_project is None:
        ...
        with st.expander("2. Показать/скрыть детали заявки"):
            # ... (Код отображения деталей без изменений) ...
            field_map = {
        ...
        if is_auth:
            ...
            try:
                current_status_index = STATUS_OPTIONS.index(current_project.get("status"))
            except ValueError:
                current_status_index = 0
            
            try:
                current_engineer_index = ENGINEER_OPTIONS.index(current_project.get("assigned_engineer"))
            except ValueError:
            ...
            with st.form("note_form", clear_on_submit=True):
                ...
                attached_files = st.file_uploader(
                    label="Прикрепить файлы к комментарию (сметы, фото и т.д.):",
                    accept_multiple_files=True,
                                ...
                                if os.path.exists(attachment["saved_path"]):
                                    with open(attachment["saved_path"], "rb") as fp:
                                        st.download_button(
                                            ...
                                            key=f"download_{attachment['saved_path']}",
                                        )
                                else:
        ...
        if not uploaded_files_info:
            ...
            for file_info in uploaded_files_info:
                ...
                st.info(f"📄 {file_name} ({file_size_mb:.2f} MB)")
